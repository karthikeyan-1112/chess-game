...
public class ChessController implements InitializingBean {
    ...
    public ResponseEntity<Map<String, Object>> botMove(@RequestBody Map<String, String> request) {
        ...
        try {
            ...
            StockfishClient sf = new StockfishClient("C:/chess/stockfish.exe"); // Change to your Stockfish path
            sf.start();
            String bestMove = sf.getBestMove(fen);
            sf.stop();

            if (bestMove != null) {
                Move move = new Move(
                    Square.valueOf(bestMove.substring(0, 2).toUpperCase()),
                    Square.valueOf(bestMove.substring(2, 4).toUpperCase())
                );
                board.doMove(move);
